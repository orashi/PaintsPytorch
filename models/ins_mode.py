...
class ResNeXtBottleneck(nn.Module):
    def __init__(self, in_channels=256, out_channels=256, stride=1, cardinality=32, dilate=1):
        ...
        D = out_channels // 2
        self.conv_reduce = nn.Sequential(nn.Conv2d(in_channels, D, kernel_size=1, stride=1, padding=0, bias=False),
                                         Inorm(D))
        self.conv_conv = nn.Sequential(nn.Conv2d(D, D, kernel_size=3, stride=stride, padding=dilate, dilation=dilate,
                                                 groups=cardinality, bias=False), Inorm(D))
...
class def_netG(nn.Module):
    def __init__(self, ngf=32):
        ...
        self.toH = nn.Sequential(nn.Conv2d(4, ngf, kernel_size=7, stride=1, padding=3),
                                 Inorm(ngf), nn.LeakyReLU(0.2, True))
