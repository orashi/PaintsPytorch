...
optimizerD2 = optim.Adam(netD2.parameters(), lr=opt.lrD, betas=(opt.beta1, 0.9))
...
if opt.optim:
    ...
    optimizerD2.load_state_dict(torch.load('%s/optimD2_checkpoint.pth' % opt.optf))
...
for epoch in range(opt.niter):
    ...
    while i < len(dataloader) - 16 // opt.batchSize:
        ...
        while j < Diters and i < len(dataloader) - 16 // opt.batchSize:
            ...
            errD_fake = netD(Variable(fake_cim[:opt.batchSize // 2]), Variable(real_sim[:opt.batchSize // 2]))[0].mean(
                0).view(1)
            errD_fake.backward(one, retain_graph=True)  # backward on score on real

            errD_real = netD(Variable(real_cim[:opt.batchSize // 2]), Variable(real_sim[:opt.batchSize // 2]))[0].mean(
                0).view(1)
            ...
            gradient_penalty = calc_gradient_penalty(netD, real_cim[:opt.batchSize // 2], fake_cim[:opt.batchSize // 2],
                                                     real_sim[:opt.batchSize // 2])
            gradient_penalty.backward()
            optimizerD.step()

            # train with D2
            errD_fake = netD2(Variable(fake_cim[opt.batchSize // 2:]), Variable(real_sim[opt.batchSize // 2:]))[0].mean(
                0).view(1)
            errD_fake.backward(one, retain_graph=True)  # backward on score on real

            errD_real = netD2(Variable(real_cim[opt.batchSize // 2:]), Variable(real_sim[opt.batchSize // 2:]))[0].mean(
                0).view(1)
            errD2 = errD_real - errD_fake

            errD_realer = -1 * errD_real + errD_real.pow(2) * opt.drift
            # additional penalty term to keep the scores from drifting too far from zero
            errD_realer.backward(one, retain_graph=True)  # backward on score on real

            gradient_penalty2 = calc_gradient_penalty(netD2, real_cim[opt.batchSize // 2:],
                                                      fake_cim[opt.batchSize // 2:], real_sim[opt.batchSize // 2:])
        ...
        if i < len(dataloader) - 16 // opt.batchSize:
            ...
            if opt.MSE:
                ...
                errd2 = netD2(fake, Variable(real_sim))
                errG = (errd.mean(0).view(1) + errd2.mean(0).view(1)) * opt.advW
                errG.backward(mone, retain_graph=True)
                contentLoss = criterion_MSE(netF((fake.mul(0.5) - Variable(saber)) / Variable(diver)),
                                            netF(Variable((real_cim.mul(0.5) - saber) / diver)))
                DMSELoss = contentLoss
        ...
        if gen_iterations < opt.baseGeni:
            ...
            writer.add_scalar('wasserstein distance 2', errD2.data[0], gen_iterations)
            ...
            writer.add_scalar('gradient_penalty 2', gradient_penalty2.data[0], gen_iterations)
    ...
    if opt.cut == 0:
        ...
        torch.save(netD2.state_dict(), '%s/netD2_epoch_only.pth' % opt.outf)
    ...
    torch.save(optimizerD2.state_dict(), '%s/optimD2_checkpoint.pth' % opt.outf)
